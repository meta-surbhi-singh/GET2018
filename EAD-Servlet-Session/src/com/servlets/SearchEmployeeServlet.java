...
public class SearchEmployeeServlet extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try (Connection conn = MyConnectionManager.getConnection();
			 PreparedStatement stmt = conn.prepareStatement(EmployeeQueries.selectWithNameQuery); ) {
		...
		out.println("<center>");	
		if(rowCount != 0) {	
		out.println("<table border = \"1\"> <tr><td>First Name</td> <td>Last Name</td> <td>Email</td> <td>age</td></tr>");	
		for (Employee employee : employeeList) {
			out.println("<tr><td>" + employee.getFirstName() + "</td> <td>" + employee.getLastName() +"</td> <td>" + employee.getEmail() +"</td> <td>" + employee.getAge() + "</td></tr>");	
		}
		out.println("</table>");
		out.println("</center>");
		}
		else {
			out.println("<h2 align = \"center\"> No Such Employee Found..!! </h2>");
		}
